...
#>
    ...
    function Scan-RMMedia {
        $ScannerPath = 'C:\Program Files (x86)\ClamWin\bin\clamscan.exe'
        cmd.exe /c "`"$ScannerPath`" --infected --recursive=yes --log=$LogPath --database=C:\ProgramData\.clamwin\db $DriveLetter"
        if ($Log) {
            Add-Content -Path $LogPath -Value "Drive $DriveLetter with label $DriveLabel scanned on $LogDate."
